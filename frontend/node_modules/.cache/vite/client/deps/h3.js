...
// node_modules/h3/node_modules/cookie-es/dist/index.mjs
...
// node_modules/h3/node_modules/radix3/dist/index.mjs
var NODE_TYPES = {
  NORMAL: 0,
  WILDCARD: 1,
  PLACEHOLDER: 2
...
var seal = async (_crypto2, object, password, options) => {
  ...
  const { encrypted, key } = await encrypt(_crypto2, pass.encryption, opts.encryption, objectString);
  const encryptedB64 = base64urlEncode(new Uint8Array(encrypted));
  const iv = base64urlEncode(key.iv);
  const expiration = opts.ttl ? now + opts.ttl : "";
  const macBaseString = `${macPrefix}*${id}*${key.salt}*${iv}*${encryptedB64}*${expiration}`;
  const mac = await hmacWithPassword(_crypto2, pass.integrity, opts.integrity, macBaseString);
